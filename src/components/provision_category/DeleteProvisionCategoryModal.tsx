import { deleteProvisionCategory } from '@/api/ProvisionCategoryAPI'
...
type DeleteProvisionCategoryModalProps = {
  id: number
  setIsOpen: (isOpen: boolean) => void
}

export default function DeleteProvisionCategoryModal({ id, setIsOpen }: DeleteProvisionCategoryModalProps) {
  const queryClient = useQueryClient()
  const { handleSubmit } = useForm()
  const [isLoading, setIsLoading] = useState(false)

  const { mutate } = useMutation({
    mutationFn: deleteProvisionCategory,
    onError: (error) => {
      setIsLoading(false)
      toast({
        title: 'Error',
