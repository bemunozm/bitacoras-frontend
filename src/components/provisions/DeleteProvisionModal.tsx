import { deleteProvision } from '@/api/ProvisionAPI'
...
type DeleteProvisionFormProps = {
  id: number
  setIsOpen: (isOpen: boolean) => void
}

export function DeleteProvisionModal({ id, setIsOpen }: DeleteProvisionFormProps) {
  const queryClient = useQueryClient()
  const { handleSubmit } = useForm()
  const [isLoading, setIsLoading] = useState(false)

  const { mutate } = useMutation({
    mutationFn: deleteProvision,
    onError: (error) => {
      setIsLoading(false)
      toast({
        title: 'Error',
